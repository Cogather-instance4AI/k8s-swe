...
const (
	...
	// Metrics to track CRI implementations that don't support RuntimeConfig
	CRILosingSupportKey = "cri_losing_support_total"
...
var (
	...
	// CRILosingSupport tracks CRI implementations that don't support RuntimeConfig when KubeletCgroupDriverFromCRI is enabled
	CRILosingSupport = metrics.NewCounterVec(
		&metrics.CounterOpts{
			Subsystem:      KubeletSubsystem,
			Name:           CRILosingSupportKey,
			Help:           "Cumulative number of CRI calls that don't support RuntimeConfig, broken down by call type.",
			StabilityLevel: metrics.ALPHA,
		},
		[]string{"operation_type"},
	)
