...
func NewEvaluator(pluginName string, fh framework.Handle, i Interface, enableAsyncPreemption bool) *Evaluator {
	...
	ev.PreemptPod = func(ctx context.Context, c Candidate, preemptor, victim *v1.Pod, pluginName string) error {
		...
		if waitingPod := ev.Handler.GetWaitingPod(victim.UID); waitingPod != nil {
			...
			// First, try to add the DisruptionTarget condition
			...
			if updated {
				if err := util.PatchPodStatus(ctx, ev.Handler.ClientSet(), victim, newStatus); err != nil {
					// If the pod is not found during status patch, it's already deleted - not an error
					if apierrors.IsNotFound(err) {
						logger.V(2).Info("Victim pod already deleted during status patch, which is acceptable", "pod", klog.KObj(victim), "preemptor", klog.KObj(preemptor))
						// Still record the event and return nil since preemption goal is achieved
			...
			// Now try to delete the pod
			if err := util.DeletePod(ctx, ev.Handler.ClientSet(), victim); err != nil {
				// NotFound error during deletion is acceptable - the pod was already deleted
				// by another process (e.g., node failure, manual deletion, other preemption)
				if apierrors.IsNotFound(err) {
					logger.V(2).Info("Victim pod already deleted, preemption goal achieved", "pod", klog.KObj(victim), "preemptor", klog.KObj(preemptor), "node", c.Name())
					// Don't return an error - the preemption goal is achieved
				} else {
					logger.Error(err, "Failed to delete victim pod during preemption", "pod", klog.KObj(victim), "preemptor", klog.KObj(preemptor))
					return err
				}
			} else {
				logger.V(2).Info("Preemptor Pod successfully preempted victim Pod", "preemptor", klog.KObj(preemptor), "victim", klog.KObj(victim), "node", c.Name())
			}
		}

		// Always record the preemption event, regardless of whether deletion was needed
		ev.Handler.EventRecorder().Eventf(victim, preemptor, v1.EventTypeNormal, "Preempted", "Preempting", "Preempted by pod %v on node %v", preemptor.UID, c.Name())
...
func (ev *Evaluator) prepareCandidate(ctx context.Context, c Candidate, pod *v1.Pod, pluginName string) *fwk.Status {
	...
	// Pre-filter victim pods to exclude those already being deleted or deleted
	validVictims := make([]*v1.Pod, 0, len(c.Victims().Pods))
	var skippedCount int
	
	for _, victimPod := range c.Victims().Pods {
		// Check if the victim Pod is already being deleted or has been deleted
		if victimPod.DeletionTimestamp != nil {
			logger.V(2).Info("Victim Pod is already being deleted, skipping", "preemptor", klog.KObj(pod), "node", c.Name(), "victim", klog.KObj(victimPod))
			skippedCount++
			continue
		}
		
		// Double-check by fetching the latest Pod state to avoid stale information
		latestPod, err := ev.PodLister.Pods(victimPod.Namespace).Get(victimPod.Name)
		if err != nil {
			if apierrors.IsNotFound(err) {
				logger.V(2).Info("Victim Pod already deleted (not found in cache)", "preemptor", klog.KObj(pod), "node", c.Name(), "victim", klog.KObj(victimPod))
				skippedCount++
				continue
			}
			// For other errors, include the pod and let PreemptPod handle it
			logger.V(4).Info("Could not get latest victim pod state, proceeding with deletion", "preemptor", klog.KObj(pod), "victim", klog.KObj(victimPod), "err", err)
		} else if latestPod.DeletionTimestamp != nil {
			logger.V(2).Info("Victim Pod is being deleted (latest state check)", "preemptor", klog.KObj(pod), "node", c.Name(), "victim", klog.KObj(victimPod))
			skippedCount++
			continue
		}
		
		validVictims = append(validVictims, victimPod)
	}
	
	// If all victims are already deleted, we still proceed to clear nominated pods
	if skippedCount > 0 {
		logger.V(2).Info("Skipped victim pods already being deleted", "preemptor", klog.KObj(pod), "node", c.Name(), "skippedCount", skippedCount, "totalVictims", len(c.Victims().Pods))
	}
	
	if len(validVictims) > 0 {
		errCh := parallelize.NewErrorChannel()
		fh.Parallelizer().Until(ctx, len(validVictims), func(index int) {
			victimPod := validVictims[index]
			if err := ev.PreemptPod(ctx, c, pod, victimPod, pluginName); err != nil {
				// Improved error handling for NotFound - don't treat as failure
				if apierrors.IsNotFound(err) {
					logger.V(2).Info("Victim Pod was deleted concurrently, which is acceptable", "preemptor", klog.KObj(pod), "victim", klog.KObj(victimPod), "node", c.Name())
					return
				}
...
func (ev *Evaluator) prepareCandidateAsync(c Candidate, pod *v1.Pod, pluginName string) {
	...
	// Pre-filter victim pods to exclude those already being deleted or deleted
	// This is done synchronously to avoid unnecessary async work
	victimPods := make([]*v1.Pod, 0, len(c.Victims().Pods))
	var initialSkippedCount int
	
	for _, victim := range c.Victims().Pods {
		// Check if the victim Pod is already being deleted
		if victim.DeletionTimestamp != nil {
			logger.V(2).Info("Victim Pod is already being deleted, skipping async deletion", "preemptor", klog.KObj(pod), "node", c.Name(), "victim", klog.KObj(victim))
			initialSkippedCount++
			continue
		}
		
		// Double-check by fetching the latest Pod state to avoid stale information
		latestPod, err := ev.PodLister.Pods(victim.Namespace).Get(victim.Name)
		if err != nil {
			if apierrors.IsNotFound(err) {
				logger.V(2).Info("Victim Pod already deleted (not found in cache), skipping async deletion", "preemptor", klog.KObj(pod), "node", c.Name(), "victim", klog.KObj(victim))
				initialSkippedCount++
				continue
			}
			// For other errors, include the pod and let the async process handle it
			logger.V(4).Info("Could not get latest victim pod state, proceeding with async deletion", "preemptor", klog.KObj(pod), "victim", klog.KObj(victim), "err", err)
		} else if latestPod.DeletionTimestamp != nil {
			logger.V(2).Info("Victim Pod is being deleted (latest state check), skipping async deletion", "preemptor", klog.KObj(pod), "node", c.Name(), "victim", klog.KObj(victim))
			initialSkippedCount++
			continue
		}
	...
	if initialSkippedCount > 0 {
		logger.V(2).Info("Skipped victim pods already being deleted during async preparation", "preemptor", klog.KObj(pod), "node", c.Name(), "skippedCount", initialSkippedCount, "totalVictims", len(c.Victims().Pods))
	}
	
	// If all victims are already deleted, cancel context and ensure pod gets requeued
	if len(victimPods) == 0 {
		logger.V(2).Info("All victim pods already deleted, requeuing preemptor pod", "preemptor", klog.KObj(pod), "node", c.Name())
		cancel()
		// Activate the pod since no async work is needed
	...
	preemptPod := func(index int) {
		...
		if err := ev.PreemptPod(ctx, c, pod, victim, pluginName); err != nil {
			// Improved error handling - only send error for non-NotFound errors
			if !apierrors.IsNotFound(err) {
				errCh.SendErrorWithCancel(err, cancel)
			} else {
				logger.V(2).Info("Victim Pod was deleted concurrently during async preemption, which is acceptable", "preemptor", klog.KObj(pod), "victim", klog.KObj(victim), "node", c.Name())
			}
		}
	}
	...
	go func() {
		...
		// Track if any victim pods were actually deleted (not already deleted)
		actualDeletionsOccurred := false
		...
		defer func() {
			// Always remove the pod from preempting set at the end
			ev.mu.Lock()
			delete(ev.preempting, pod.UID)
			ev.mu.Unlock()

			// Activate the preemptor Pod in these cases:
			// 1. When API call isn't successful
			// 2. When no actual deletions occurred (all pods were already deleted)
			//    This ensures the preemptor doesn't get stuck
			if result == metrics.GoroutineResultError || !actualDeletionsOccurred {
				logger.V(2).Info("Activating preemptor pod after async preemption", "preemptor", klog.KObj(pod), "reason", func() string {
					if result == metrics.GoroutineResultError {
						return "error occurred"
					}
					return "no actual deletions occurred"
				}())
				ev.Handler.Activate(logger, map[string]*v1.Pod{pod.Name: pod})
			}
		}()
		...
		if len(victimPods) > 1 {
			...
			case apierrors.IsNotFound(err):
				logger.V(2).Info("Some victim pods were already deleted during parallel eviction", "preemptor", klog.KObj(pod), "node", c.Name(), "err", err)
			case err != nil:
				utilruntime.HandleErrorWithContext(ctx, err, "Error occurred during async preemption")
				result = metrics.GoroutineResultError
			default:
				actualDeletionsOccurred = true
			}
		}

		// Process the last victim pod
		err := ev.PreemptPod(ctx, c, pod, victimPods[len(victimPods)-1], pluginName)
		switch {
		case apierrors.IsNotFound(err):
			logger.V(2).Info("Last victim Pod was already deleted", "preemptor", klog.KObj(pod), "node", c.Name(), "err", err)
		case err != nil:
			utilruntime.HandleErrorWithContext(ctx, err, "Error occurred during async preemption for last victim")
			result = metrics.GoroutineResultError
		default:
			actualDeletionsOccurred = true
		}

		logger.V(2).Info("Async Preemption finished completely", "preemptor", klog.KObj(pod), "node", c.Name(), "result", result, "actualDeletions", actualDeletionsOccurred)
	}()
}
